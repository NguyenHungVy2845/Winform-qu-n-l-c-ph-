...
namespace QuanLyCafe
{
    public partial class TableManager : Form
    {
        ...
        // --- CÁC MENU ---
        // Menu: Hệ thống bàn (Chỉ Admin được vào sửa bàn)
        private void HệthốngbànMenu_Click(object sender, EventArgs e)
        {
            ...
            // 2. Kiểm tra quyền Admin (LoaiTaiKhoan = 1)
            if (Session.CurrentUser.LoaiTaiKhoan == 1)
            {
                QuanLy f = new QuanLy();
                f.SelectTabByTitle("Bàn ăn"); // Mở đúng tab Bàn ăn bên form Quản lý

                this.Hide();
                f.ShowDialog();
                this.Show();

                // Load lại dữ liệu phòng khi Admin có sửa đổi gì đó
                LoadTable();
                LoadCategory();
            }
            else
            {
                MessageBox.Show("Bạn không có quyền truy cập vào Hệ thống bàn!", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        private void QuyềnquảnlýToolStripMenuItem_Click(object sender, EventArgs e)
        {
            // 1. Kiểm tra session đăng nhập (để chắc chắn biến không bị null)
            if (Session.CurrentUser == null)
            {
                MessageBox.Show("Vui lòng đăng nhập lại hệ thống!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            // 2. Kiểm tra Loại tài khoản (1 = Admin/Quản lý, 0 = Nhân viên)
            // Logic này dựa trên quy ước trong Database của bạn (LoaiTaiKhoan: 0=NV, 1=Admin)
            if (Session.CurrentUser.LoaiTaiKhoan == 1)
            {
                QuanLy f = new QuanLy();
                this.Hide();
                ...
                // Load lại dữ liệu bàn và danh mục sau khi đóng form quản lý
                // (phòng trường hợp bên kia có chỉnh sửa gì đó)
                LoadTable();
                LoadCategory();
            }
            else
            {
                MessageBox.Show("Bạn không có quyền truy cập vào chức năng này!\nChức năng chỉ dành cho Quản lý.",
        ...
        // Menu: Thống kê (Chỉ Admin được xem doanh thu)
        private void ThốngkêToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Bạn không có quyền xem Thống kê doanh thu!", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        {
            // Mở form Liên hệ dạng hộp thoại (không thao tác được form chính khi chưa tắt form này)
